...
generatePaths <- function(base.dir, T, P, stringFormat = "[%s,%s]", times = FALSE) {
  ...
  for (i in 1:length(T)) for (j in 1:length(P)) {
    ...
    titles[k] <- paste0(T[i], ", ", P[j])
    if (times)
      titles[k] <- paste0(titles[k], " (", last(x$times), ")")
  }
...
panelPlot <- function(datafiles, nrow, ncol, titles = "", dim = 2, cex = 0.5, omi = c(0.5,
...
plotSeedRain <- function(res, xlim = c(0, 175), ylim = c(0, 0.05), cols = 1:100,
  ytck = seq(0, 0.05, 0.01), xtck = seq(0, 200, by = 50), xlab = expression(paste("Time since disturbance (yr)")),
  ylab = expression(paste("Seed production (", yr^-1, ")"))) {
...
plotSizeDensityDistribution2D <- function(res, cols = 1:10, xlim = c(0, 180), ylim = c(0,
  35), xtck = seq(0, 200, by = 50), ytck = seq(0, 40, by = 10), xlab = expression(paste("Time since disturbance (yr)")),
...
figure2 <- function() {
  ...
  landscape.hsp <- file.path(dir1, "hsp/[0.625]/base/T1-1000-1D.txt")
  ...
  headerplot <- function(label, x, y = 1.2) {
  ...
  headerplot("A", "Only leaf mass per unit leaf area evolving")
  ...
  headerplot("B", "Only height at maturation evolving")
  ...
  headerplot("C", "Both traits evolving together")
...
figureSI <- function() {
  ...
  mylabel <- function(label, x, y = 1.08) {
    label(-0.2, y, label, cex = 1.05, xpd = NA, font = 2)
    label(-0.1, y, x, cex = 1.05, xpd = NA, font = 1)
  }

  HLIM <- log10(c(0.5, 80))

  par(mfrow = c(1, 3), oma = rep(2, 4), mar = c(5.1, 4.1, 4, 3.1))
  ...
  emptyTraitsPlot(ylim = HLIM, ylab = "", xlab = "")
  plotViability(viability)
...
figure3 <- function() {
  ...
  steps <- c(0, 1, 6, 10, 400, 2000, 3000, 5000)

  V <- read.table("output/finalCommunities/[60,1]/2trait/base/Viability.txt",
    sep = "\t")[, 2:4]

  par(oma = c(5, 5, 3, 8), mar = c(1, 1, 3, 1))

  layout(matrix(c(1, 2, 3, 4, 10, 5, 6, 7, 8, 9), 2, 5, byrow = TRUE), c(3, 3,
    3, 3, 1), c(1, 1))
  ...
  for (i in 1:8) {
    ...
    plotFitnessLandscapeFromFile(sprintf("output/data/[60,1]/2trait/base/T-%d-2D.txt",
      steps[i]), dim = 2, title = "", zlim = c(-6, 6), Ylab = (i %in% c(1,
      5)), Ytitle = 0, Xtitle = 0, Xlab = (i > 4), line = 3, ylim = log10(c(0.5,
...
figS_Assembly_height <- function(path) {
  times <- c(0:20, seq(40, 200, by = 20), seq(200, 1000, by = 200))
  x <- list(times = times, paths = fitnessFileNameFormat(path,
    times, dim = 1, Tsuffix = "1"))
  plotAssembly("", ncol = 5, dim = 1, xlim = c(-1, 2), x = x, zlim=c(-4,4), col_pallete_fn = function(x,
    y) {
    "white"
  })
}

figS_Assembly_lma <- function() {
  times <- c(0:20, seq(40, 200, by = 20), seq(200, 1000, by = 200))
  x <- list(times = times, paths = fitnessFileNameFormat("output/data/[60,1]/lcc/[1.3725]/base",
...
figure_sensitivity_1 <- function() {
  ...
  panelPlot(x$paths, nrow = 5, ncol = 6, gsub(" (5000)", "", x$titles, fixed = TRUE),
...
figure_sensitivity_2 <- function() {
  ...
  panelPlot(x$paths, nrow = length(T), ncol = length(P), x$titles, dim = 2, cex = 1.2,
...
figure_sensitivity_3 <- function() {
  ...
  for(i in seq_along(paths)) {
    ...
    label(-0, 1.15, paste0(toupper(letters[i]), ", ",  r1[i]), cex = 1.5, xpd=NA)
  }
}
...
figure5 <- function(data) {
  ...
  mtext("                                                     Time since disturbance (yr)",
    side = 1, line = 3, outer = TRUE, cex = 1.25)
  mtext("Average disturbance interval (yr)", side = 2, line = 3, outer = TRUE,
    cex = 1.25)
...
figure5_single <- function(data) {
  ...
  mtext("Average disturbance interval (yr)", side = 2, line = 3, cex = 2)
...
figure_plot_patch <- function(data) {
  ...
  plot_stand(data$spatial, size = c(1, 1, 1500 * data$scale, 500))
  ...
  lines3d(rep(coords$x[1], 2), c(y0, y0), c(0, 5), col = "red")
  lines3d(rep(coords$x[1], 2) - c(0, 0.5) * vadj, c(y0, y0), c(0, 0), col = "red")
  lines3d(rep(coords$x[1], 2) - c(0, 0.5) * vadj, c(y0, y0), c(5, 5), col = "red")
  text3d(coords$x[1] - 1 * vadj, y0, 5, "5m", adj = c(0.75, 0), col = "black",
    cex = 0.75)
  ...
  for (ii in 1:3) {
    ...
    lines3d(rep(t, 2), c(0, -1) * vadj, c(0, 0), col = "black")
    text3d(t, -3 * vadj, 0, lab[ii], adj = c(0.5, 0), col = "black")
  }
}
...
fig_2D_landscapes_all <- function() {
  ...
  text(4, 12, srt = -90, pos = 1, labels = "Disturbance interval (yr)", xpd = NA,
...
figure1_LAI <- function(Res, cols = defaultColours()) {
  ...
  headerplot("A", "Temporal dynamics within a patch", 2.3)
  ...
  mtext("Time since disturbance (yr)", side = 1, cex = 1, line = 3)
  axis(2, at = 0:5, labels = 0:5, las = 1)
  mtext("Leaf area per ground area", side = 2, cex = 1, line = 3)
  axis(4, at = c(0, 0.25, 0.5, 0.75, 1) * ymax, labels = c(0, 25, 50, 75, 100),
    las = 1)
  text(450, 2.5, srt = -90, pos = 1, labels = "Patch survival (%)", xpd = TRUE,
    cex = 1.3)
...
growth_plot <- function() {
  ...
  mtext(expression(paste("Leaf mass per unit leaf area (kg ", m^-2, ")")), line = 3,
    side = 1, cex = 1)
  mtext(expression(paste("Height growth rate (m ", yr^-1, ")")), line = 3, side = 2,
    cex = 1, col = "red")
